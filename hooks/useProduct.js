...
function getImageURL(source, size) {
    ...
    if (source) {
        //alert (source.ImageUrl);
        if (size && size === 'large') {
            if (source.formats.large) {
                image = source.formats.large.ImageUrl;
            } else {
                image = source.ImageUrl;
            }
        } else if (size && size === 'medium') {
            if (source.formats.medium) {
                image = source.formats.medium.ImageUrl;
            } else {
                image = source.ImageUrl;
            }
        } else if (size && size === 'thumbnail') {
            if (source.formats.thumbnail) {
                image = source.formats.source.ImageUrl;
            } else {
                image = source.ImageUrl;
            }
        } else if (size && size === 'small') {
            if (source.formats.small !== undefined) {
                image = source.formats.small.ImageUrl;
            } else {
                image = source.ImageUrl;
            }
        } else {
        ...
        //imageURL = `http://localhost:16595${image}`;
    } else {
...
export default function useProduct() {
    return {
        ...
        price: (payload) => {
            let view;
           // console.log("PRICE:"+JSON.stringify(payload));
        ...
        title: (payload) => {
            let view = (                
                <Link href="/product/[pid]" as={`/product/${payload.SeName}`}>
